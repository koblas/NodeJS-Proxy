...
function _do_update(data, curpos) {
    for (var idx in data) {
        ...
        if (block.op == 'dialog_close') {
            ...
            $.facebox(dialog, block.dialogClass);
            openDialog = true;
...
function _bootstrap(html) {
    ...
    findself('form').each(function() {
        ...
        function handle_submit(event) {
            ...
            */
            ...
            if (cookie) {
                var csrf_input = $("input[name='csrfmiddlewaretoken']", self);

                if (!csrf_input || csrf_input.size() == 0) {
                    csrf_input = $("<input>").attr("type","hidden").attr("name","csrfmiddlewaretoken");
                    form.append(csrf_input);
                }

                csrf_input.val(cookie);
            }
        ...
        function handle_submit_csrf(event) {
            var cookie = $.cookie('csrftoken');
            
            if (cookie) {
                var input = $("<input>").attr("type","hidden").attr("name","csrfmiddlewaretoken").val(cookie);
                form.append(input);
            }
        ...
        if ($(this).attr('rel') == 'async') {
            ...
            form.bind('submit', handle_submit_csrf);
        }
    });
    ...
    findself('.closeable').each(function () {
        ...
        span.click(function () {
            if (self.attr('href')) {
                var href = self.attr('href').split('#_')[1]
...
function load_body(url) {
    $(document).ready(function() { _do_update([{'op':'redirect', 'url': url }]) });
}
